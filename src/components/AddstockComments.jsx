...
import { GetApiDataUpdate } from './ApiCalls';
...
const initSelfForm = getDBValue("STOCKFORM");
...
const handleSubmit = (props) => {
    ...
    const loc = selfForm[selfForm.findIndex( a => a.name === "STOCK_LOC")].value;
    const comments = selfForm[selfForm.findIndex( a => a.name === "Comments")].value;
    const sql = `update deliveries set stock_loc='${loc}',comments=nvl2(comments,comments||'-'||'${comments}','${comments}'),stock_upd_by='${accountInfo.email}' where order_id=${orderId}`;

    GetApiDataUpdate(sql)
    .then((res) => {
        // console.log(res)
        if (res > 0 ){
            toast.success("Stock comments Updated");
            history.goBack();
        }
        else {
            toast.error( `Error !!!`) ;
